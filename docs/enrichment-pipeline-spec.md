...
- **Tier 1 override, identity mismatch (found 2026-08-09, now a hard rule).** Even when the mechanical criteria above are met, if the launch verification source (company register, Handelsregister, Companies House, etc) names a different person as founder/officer/director than the contact on file, do not auto-enroll. Route to `MANUAL_REVIEW` instead and note the mismatch in `businessLaunchSource`. A confident launch date attached to the wrong person is worse than an unconfident one, since the message explicitly congratulates "you" on launching the business.
- **Tier 1 override, no verifiable business purpose (found 2026-08-09, now a hard rule).** Stage 4's opener requires a specific, genuine synthesis of what the business does. If Stage 1 and Stage 2 research (including the live site itself) turn up no independently verifiable description of what the business actually does — this can happen even on a clean PLACEHOLDER/BASIC site read, e.g. a holding page with no copy at all and no matching description anywhere else — do not write the opener from a guess. Route to `MANUAL_REVIEW` instead of INCLUDE, even if every other Tier 1 criterion is met, and note the gap so a human can supply the missing description.
- **Tier 2 — weekly batch approval.** `businessLaunchStatus = DO_NOT_USE` (no real evidence the business is new at all), `MANUAL_REVIEW` eligibility rows (including either override above), or LOW website confidence. Accumulate these into a single weekly digest (one list, not per contact prompts) for a single go/no go from Raka, then import the approved batch.
- **EXCLUDE** rows are never imported, automatically or otherwise.
- Rationale for the split: a wrong launch date or a misread website can't be unsent once the connect message goes out, so the confidence signals already being computed in Stage 1/2 double as the automation gate — no extra logic needed, just a threshold on data already produced. The two overrides above exist because the mechanical confidence fields alone don't capture identity mismatch or fabrication risk, which are just as unsendable as a wrong launch date.
